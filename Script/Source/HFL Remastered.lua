...
class 'init'
			...
			if self:checkAccess() then
				if editMode then
					...
					if _G[myHero.charName] then
						local heroPlugin = _G[myHero.charName]()
						self.pluginName = heroPlugin.name
					else
						local heroPlugin = _Default()
						self.pluginName = heroPlugin.name
					end
	...
	function init:drawInfo()
		...
		if(_G[myHero.charName]) then
			DrawText("Profile: " .. self.pluginName, 18, 60, 170, ARGB(255,0,255,0))
		else
			DrawText("Profile: " .. self.pluginName, 18, 180, 170, ARGB(255,220,100,0))
		end
...
class '_Recall'
	...
	function _Recall:check()
		...
		if (myHero.health * 100)/myHero.maxHealth < 20 and not AutoItem:potExists() then
			self.valid = true
		end
	end
...
class '_Survive'
	...
	function _Survive:check()
		...
		if (myHero.health * 100) / myHero.maxHealth < 40 then
	...
	function _Orbwalker:CanOrbwalkTarget(Target)
	    if ValidTarget(Target) then
            if Target.type == myHero.type then
            	...
            	if Target.type == "obj_HQ" then
	                if Helper:GetDistance(Target) - 100 + 20 < MyHero.TrueRange then
	                    return true
	                end
	            else
	...
	function _Remote:manager(cmd)
		...
		if cmd[1] == "login" then
			...
			if string.find(loginStatus,"Success") then
				...
				if not itemBuilder or not itemBuilder() then
					PrintSystemMessage("Failed to fetch item build, resending packet.")
					DelayAction(function() 
						self:login()
					end, 1.5)
				else
...
class '_Default'
	function _Default:__init()
		...
		AddTickCallback(function ()
			...
			if state == PUSH_STATE then
				self:push()
			end
			if state == SURVIVAL_STATE then
				self:survival()
			end
			if state == COMBAT_STATE then
				self:combat()
			end
			if state == HOLDLANE_STATE then
				self:holdlane()
			end
			if state == HARASS_STATE then
				self:harass()
			end
		end)
		if not SpellData.spellData then
