			...
			if not VIP_USER then
				for i=0, 500 do
					DelayAction(function() 
						if GetDistance2D(_G.hflTasks[MAPNAME][TEAMNUMBER][1].point,myHero) > 4000 then
							CastSpell(RECALL) 
						end
					end, 0.01 * i)
				end
	...
	function _Recall:amIsafe()
		...
		for k, enemy in pairs(Helper.EnemyTable) do
			if not UnitHaveBuff(myHero, "recall") then
				...
				if GetDistance2D(enemy,myHero) < 300 and not enemy.dead and enemy.visible then
					dangerFree = false
				end
			end
		end
	...
	function _Remote:playerStats()
		if self.lastPing + 3 < GetInGameTimer() then
			...
			if WINDOW_TITLE then
				packet = table.concat({"ping",GetInGameTimer(),myHero.level,myHero:GetInt('CHAMPIONS_KILLED'),myHero:GetInt('NUM_DEATHS'),myHero:GetInt('ASSISTS'),myHero:GetInt('MINIONS_KILLED'),myHero.gold,myHero.x,myHero.z,MAPNAME,WINDOW_TITLE}, "|")
			else
				packet = table.concat({"ping",GetInGameTimer(),myHero.level,myHero:GetInt('CHAMPIONS_KILLED'),myHero:GetInt('NUM_DEATHS'),myHero:GetInt('ASSISTS'),myHero:GetInt('MINIONS_KILLED'),myHero.gold,myHero.x,myHero.z,MAPNAME,"NOT UPDATED"}, "|")
			end
			self.Socket:send(packet)
			self.lastPing = GetInGameTimer()
		end
			...
			if target then
				if spellSettings.onlykill == "true" then
					...
					if SpellData.spellData[spellSettings.key].speed and SpellData.spellData[spellSettings.key].delay then
						...
						predhealth, _, count = VP:GetPredictedHealth(target, time)
						if dmg > predhealth then
							if SpellData.spellData[spellSettings.key].aoe then
								local CastPosition, HitChance, Position = Predict(SpellData.spellData[spellSettings.key],target)
								if HitChance >= 2 then
									CastSpell(spellSettings.key,CastPosition.x,CastPosition.z)
								end
							else
								local CastPosition, HitChance, maxHit, Position = Predict(SpellData.spellData[spellSettings.key],target)
								if HitChance >= 2 then
									CastSpell(spellSettings.key,CastPosition.x,CastPosition.z)
								end
							end
